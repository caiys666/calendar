        ...
        <div class="list_item" v-for=" (item, index) in daylist" :key="index" @mouseover="addClass(index)">
            ...
            <div class="changeSchedule">
                <div v-for="(subitem, subindex) in schduleListArray" :key="subindex">
                    <span v-for="(ssubitem,ssindex) in subitem.flagList" @click="changeSchedule(ssubitem)">
                        <span v-if="(index === ssubitem.flagIndex)">{{ssubitem.title}}</span>
                    </span>
                </div>
            </div>
        </div>
        <div v-if="show_dialog.status">
            <Dialog v-model="show_dialog.status" :indexes="show_dialog.index" :timestamp="show_dialog.timestamp" type="confirm" title="新建日程" content="我是内容" @cancel="clickCancel()" @danger="clickDanger()" @confirm="clickConfirm" dangerText="Delete"></Dialog>
        </div>
    </div>
</div>
</template>
...
export default {
    ...
    methods: {
        ...
        changeShowDate(year, month, thisday) {
            ...
            this.timestampList = new Array(totalday1 - 1);
            this.day = [];
            this.daylist = []
        ...
        addDialog(index) {
            ...
            this.timestampList.splice(index, 0, {
                t_timestamp: this.show_dialog.timestamp,
            })
            for (let i in this.timestampList) {
                console.log(i)
            }
            console.log(this.timestampList.t_timestamp)
            // console.log(this.show_dialog.timestamp)
            // this.isactive = index
        },
        addClass(index) {
