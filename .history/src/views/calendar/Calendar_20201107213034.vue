...
<div class="Calendar">
    ...
    <div id="list" class="list">
        <div class="list_item" v-for="(item, index) in day" :key="index" @click="addDialog">
            <div :id="item">{{ item }}</div>
            <div class="list_item_dialog"></div>
        </div>
        <div v-show="show_dialog" class="show_dialog">
            <Dialog v-model="show_dialog" type="confirm" title="新建日程" content="我是内容" @cancel="clickCancel()" @danger="clickDanger()" @confirm="clickConfirm" dangerText="Delete"></Dialog>
        </div>
    </div>
</div>
</template>
    ...
    methods: {
        ...
        changeShowDate(year, month) {
            ...
            for (
                ...
                this.day.push(j);
                this.daylist.index = j;
                this.daylist.title = year + month + j
            }
            for (let i = 0; i < totalday; i++) {
                this.day.push(i + 1);
                this.daylist.index = i + 1;
                this.daylist.title = year + month + i + 1
            }
            for (let i = 1; i <= 6 - PreLastDay.lastMothLastDay1; i++) {
                this.day.push(i);
                this.daylist.index = i;
                this.daylist.title = year + month + i
            }
        ...
        addDialog() {
            ...
            list.style.background = 'rgb(243, 53, 53)'
            item.style.background = 'rgb(243, 53, 53)'
        },
        openMask(index) {
            this.show_dialog = true;
        },
