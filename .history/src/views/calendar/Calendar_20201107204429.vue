...
<div class="Calendar">
    ...
    <div class="list">
        <div class="list_item" v-for="(item, index) in day" :key="index" @click="addDialog(item)">
            <div>{{ item }}</div>
            <div class="list_item_dialog"></div>
        </div>
        <div v-show="show_dialog" class="show_dialog">
            <Dialog v-model="show_dialog" type="confirm" title="新建日程" content="我是内容" @cancel="clickCancel()" @danger="clickDanger()" @confirm="clickConfirm(dialogContent)" dangerText="Delete"></Dialog>
        </div>
    </div>
</div>
</template>
...
export default {
    ...
    methods: {
        ...
        clickConfirm(dialogContent) {
            console.log('点击了confirm' + dialogContent.title);
        }
    },
    mounted() {
        this.showDate();
