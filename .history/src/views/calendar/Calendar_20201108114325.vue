    ...
    <div id="list" class="list">
        <div class="list_item" v-for=" (item, index) in daylist" :key="index" @mouseover="addClass(index)">
            ...
            <div class="changeSchedule">
                <div v-for="(item, subindex) in  schduleListArray.flaglist" :key="subindex" @click="changeSchedule(item)">
                    <span v-if="index === item.flagIndex">日程信息{{item}}</span>
                </div>
            </div>
        </div>
...
export default {
    ...
    methods: {
        ...
        clickConfirm(data) {
            ...
            this.schduleListArray.push(data);
            console.log(this.schduleListArray[0].flaglist[0].title)
        }
    },
    mounted() {
        this.showDate();
