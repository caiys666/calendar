            ...
            <div class="changeSchedule">
                <div v-for="(subitem, subindex) in schduleListArray" :key="subindex">
                    <span v-for="(ssubitem,ssindex) in subitem.flagList" @click="changeSchedule(ssubitem)">
                        <span v-if="index === ssubitem.flagIndex">日程信息{{ssubitem.title}}</span></span>
                </div>
            </div>
        </div>
...
export default {
    ...
    methods: {
        ...
        clickConfirm(data) {
            ...
            for (let i in this.schduleListArray) {
                console.log(this.schduleListArray[i])
                for (let j in this.schduleListArray[i]) {
                    console.log(this.schduleListArray[i][j])
                }
            }
        }
    },
