...
<div class="Calendar">
    ...
    <div id="list" class="list">
        <div class="list_item" v-for=" (item, index) in daylist" :key="index" @click="addDialog(index)">
...
export default {
    ...
    methods: {
        ...
        changeShowDate(year, month, thisday) {
            ...
            let this_index = this.searchKeys(thisday, this.daylist, month);
        ...
        searchKeys(thisday, daylist, month) {
            let result = [];
            for (let i = 0; i < daylist.length; i++) {
                if (thisday == daylist[i].id && daylist[i].index == 'thismonth' && daylist[i].month == month) {
                    result.push(i);
                }
            }
            return result[0];
