    ...
    <div id="list" class="list">
      ...
      >
        <div
          :class="{
            itemtext,
            list_item_test: isactive === index ? true : false,
          }"
        >
          <span>{{ item.id }}</span>
        </div>
        <div
          v-if="addClassTag === index"
          @click="addDialog(index)"
          class="list_item_dialog"
        >
          +
        </div>
        <div class="changeSchedule">
          <div v-for="(subitem, subindex) in schduleListArray" :key="subindex">
            <span
              v-for="(ssubitem, ssindex) in subitem.flagList"
              @click="changeSchedule(ssubitem, subindex)"
            >
              <span
                :class="{
                  changeItem,
                  red: ssubitem.importance == 1 ? true : false,
                  blue: ssubitem.importance == 2 ? true : false,
                  green: ssubitem.importance == 3 ? true : false,
                }"
                v-if="
                  index === ssubitem.flagIndex &&
      ...
      <div v-if="show_dialog.status">
        <Dialog
          v-model="show_dialog.status"
          :itemDialog="itemDialog"
          :indexes="show_dialog.index"
          :timestamp="show_dialog.timestamp"
          type="confirm"
          title="新建日程"
          content="我是内容"
          @cancel="clickCancel()"
  ...
  methods: {
    ...
    changeShowDate(year, month, thisday) {
      ...
      for (
        ...
        i <= nextMonthDays.nextMonthFirstDay1;
        i++, j++
      ) {
        this.day.push(j);
        this.daylist.push({
      ...
      let this_index = this.searchKeys(year, thisday, this.daylist);
    ...
    searchKeys(thisyear, thisday, daylist) {
      let result = [];
      for (let i = 0; i < daylist.length; i++) {
        if (
          thisday == daylist[i].id &&
          new Date().getFullYear() == daylist[i].year &&
          daylist[i].index == "thismonth" &&
          daylist[i].month == this.thismonth
        ) {
          result.push(i);
        }
      }
